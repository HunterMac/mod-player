...
function Main() {
  ...
  const onListDirClick = (id, name) => {
    ...
    breadcrumbs.push(
      <Link
        ...
        color="inherit"
        sx={breadcrumbLinkStyle}
        onClick={() => onBreadcrumbClick(id, itemIndex)}
      >
        {" "}
        {name}{" "}
      </Link>
  ...
  const renderList = (nodes) => {
    return nodes.map((node) => {
      return (
        <ListItem key={node.id} disablePadding dense>
          <ListItemButton
            onClick={() =>
              node.isDir
                ? onListDirClick(node.id, node.name)
                : setModuleInfo({
                    url: gapiManager.getFileUrl(node.id),
                    fileName: node.name,
                  })
            }
          >
      ...
      <header className="App-header">
        <Grid container spacing={2}>
          <Grid container item xs={12} md={7} direction="column">
            ...
            <List sx={fileListStyle}>{renderList(fileList)}</List>
          </Grid>
          <Grid container item xs={12} md={4} direction="column">
            <PlayControls moduleInfo={moduleInfo} />
            <Button variant="contained" onClick={gapiManager.signIn}>
              Google Drive Login
            </Button>
          </Grid>
        </Grid>
